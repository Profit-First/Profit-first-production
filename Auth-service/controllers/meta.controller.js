...
class MetaController {
  ...
  async handleCallback(req, res) {
    try {
      ...
      await this.saveConnection(userId, accessToken, profile, adAccounts, expiresIn);
  ...
  async selectAdAccount(req, res) {
    try {
      ...
      // Fetch 3 months of insights data for the selected ad account
      console.log(`📊 Fetching 3 months of insights data...`);
      await metaSyncService.fetch3MonthsData(userId);
  ...
  /**
   * Save Meta connection to database
   */
  async saveConnection(userId, accessToken, profile, adAccounts, expiresIn) {
  ...
  /**
   * Fetch 3 months of insights data for selected ad account
   */
  async fetch3MonthsInsights(userId, accessToken, adAccounts) {
    ...
    try {
      ...
      for (const account of adAccounts) {
        ...
        try {
          ...
          console.error(`   ⚠️  Error fetching insights for ${numericAccountId}:`, error.message);
      ...
      console.log(`   ✅ 3 months insights data fetched and stored`);

    } catch (error) {
      console.error(`   ❌ Error fetching 3 months insights:`, error.message);
      // Don't throw - we don't want to break the ad account selection
    }
  }

  /**
   * Save individual insight data to database
   */
  async saveInsightData(userId, adAccountId, insight) {
    ...
    const dateAccount = `${date}#${adAccountId}`; // Composite sort key

    const item = {
      userId,
      adAccountId,
      date,
      dateAccount, // Composite key for querying
