...
function calculateSummary(orders, products, metaInsights, shiprocketShipments) {
  ...
  // If Shiprocket data has no costs, estimate based on orders
  // Average shipping cost in India: ₹70 per order
  if (shippingCost === 0 && orders.length > 0) {
    const avgShippingPerOrder = 70;
    shippingCost = orders.length * avgShippingPerOrder;
    console.log(`⚠️  No shipping cost data from Shiprocket, estimating: ${orders.length} orders × ₹${avgShippingPerOrder} = ₹${shippingCost}`);
  }
...
function calculateWebsiteMetrics(orders) {
  const sessions = orders.length * 3; // Rough estimate
  const conversionRate = orders.length > 0 ? (orders.length / sessions) * 100 : 0;
...
function calculateOrderTypeData(orders) {
  ...
  const prepaid = orders.filter(o => o.financialStatus === 'paid').length;
  const cod = orders.length - prepaid;
