...
const processChatQuery = async (userId, query) => {
  try {
    ...
    const addMetric = (label, value, isPercentage = false, isCurrency = true) => {
      ...
      // Skip if not a valid number or is zero
      if (isNaN(numValue) || numValue === 0 || numValue === null || numValue === undefined) return;

      let formattedValue;
      if (isPercentage) {
        formattedValue = `${numValue.toFixed(2)}%`;
      } else if (isCurrency) {
        formattedValue = `₹${numValue.toLocaleString('en-IN', { maximumFractionDigits: 0 })}`;
      } else {
